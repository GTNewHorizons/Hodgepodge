    ...
    public void redirectSerialize(PacketBuffer instance, String s) {
        ...
        if (bytes.length > 32767) {
            if (Hodgepodge.config.logHugeChat) {
                ...
                bytes = Serializer.func_150696_a(new ChatComponentText(
                        EnumChatFormatting.RED + "Someone tried to sent you a huge chat message that would kick you. " +
                                "Ask your server admin to see details." +
                                " Please provide these info while reporting: Incident ID " + EnumChatFormatting.UNDERLINE + incidentId + EnumChatFormatting.RESET +
                                ". Start of original message: " + getMessage().getUnformattedText().substring(0, 30)))
                        .getBytes(StandardCharsets.UTF_8);
            } else {
                LOGGER.info("HUGE chat message caught. Details are not logged here as requested in config.");
                bytes = Serializer.func_150696_a(new ChatComponentText(
                        EnumChatFormatting.RED + "Someone tried to sent you a huge chat message that would kick you. " +
                                "Ask your server admin to turn on logHugeChat in HodgePodge if you cannot identify the source of this and this keeps happening. " + EnumChatFormatting.RESET +
                                "Start of original message: " + getMessage().getUnformattedText().substring(0, 30))).getBytes(StandardCharsets.UTF_8);
            }
        }
